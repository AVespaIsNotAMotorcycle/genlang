/*
string_keys_hashing.hpp
Sam Ebersole
Defines a hash table using double hashing on StringKeys. 
Modified from an assignment for CSCI33500.
November 6, 2020
*/

#ifndef STRING_KEYS_HASHING_H
#define STRING_KEYS_HASHING_H
...
//template <typename StringKeys>
class HashTableStringKeys {
 public:
  enum EntryType {ACTIVE, EMPTY, DELETED};

  explicit HashTableStringKeys(size_t size = 101) : array_(NextPrime(size))
    { MakeEmpty(); }

  bool Contains(const StringKeys & x) const {
  ...
  bool Remove(const StringKeys & x) {
    size_t current_pos = FindPos(x);
    if (!IsActive(current_pos))
      return false;
  ...
  StringKeys GetEntry(const StringKeys & x) {
    int probes = 1;
 ...
 private:
  ...
  size_t FindPos(const StringKeys & x) const {
    size_t current_pos = InternalHash(x);
    size_t double_hash = DoubleHash(x);
  ...
  size_t InternalHash(const StringKeys & x) const {
    static std::hash<string> hf;
    return hf(x.getName()) % array_.size( );
  }

  size_t DoubleHash(const StringKeys & x) const {
    static std::hash<string> hf;
    return r_value_ - (hf(x.getName()) % r_value_);
  }
